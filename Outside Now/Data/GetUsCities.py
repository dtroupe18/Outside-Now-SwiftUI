...
def load_csv_as_df(file_name, sub_directories, col_names=None):
    '''
    Load any csv as a pandas dataframe.
    Provide the filename, the subdirectories, and columns to read (if desired).
    '''
    base_path = os.getcwd()
...
# FIXME: sort the cities names before saving to csv so we can optimize if a failure occurs
